...
RT_PROGRAM void closestHit()
{
    ...
    float3 omega_i = s.x * u + s.y * v + s.z * w;
    ...
    if (cf.importanceSamplingMode == 0) {
        payload.throughput *= 2 * M_PIf * f * nDotwi;
    }
    else if (cf.importanceSamplingMode == 1) {
        payload.throughput *= M_PIf * f;
    }
    else if (cf.importanceSamplingMode == 2) {
        if (mv.BRDFMode == 0) {
            float pdf = ((1 - t) * nDotwi / M_PIf) + (t * ((mv.shininess + 1) / (2 * M_PIf)) * pow(rDotwi, mv.shininess));
            payload.throughput *= f * nDotwi / pdf;
        }
        else {
            float theta_h = acos(dot(h, attrib.normal));
            float D = pow(mv.roughness, 2.0f) / (M_PIf * pow(cos(theta_h), 4.0f) * pow((pow(mv.roughness, 2.0f) + pow(tan(theta_h), 2.0f)), 2.0f));
            float pdf = ((1 - t) * nDotwi / M_PIf) + (t * D * dot(h, attrib.normal)/ (4 * dot(h, omega_i)));
            payload.throughput *= f * nDotwi / pdf;
        }
